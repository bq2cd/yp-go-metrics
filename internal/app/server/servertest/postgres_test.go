...
func TestLaunchEmbeddedPostgresWithDelay(t *testing.T) {
	type args struct {
		t        *testing.T
		user     string
		password string
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			got := LaunchEmbeddedPostgresWithDelay(tt.args.t, tt.args.user, tt.args.password, tt.args.dbname, tt.args.delay)
			assert.Equal(t, tt.want.got, got)
		})
	}
}
